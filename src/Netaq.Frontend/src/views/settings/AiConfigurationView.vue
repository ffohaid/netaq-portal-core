...
const reverseProviderTypeMap: Record<string, number> = {
  'Gemini': 1,
  'Ollama': 2,
  'Aya': 3,
  'OpenAI': 4,
...
const providerTypes = [
  { value: 1, label: 'Google Gemini' },
  { value: 2, label: 'Ollama (Local)' },
  { value: 3, label: 'Aya (Local)' },
  { value: 4, label: 'OpenAI Compatible' },
]
...
function openEditModal(config: AiConfiguration) {
  ...
  form.value = {
    providerType: typeof config.providerType === 'string' ? (reverseProviderTypeMap[config.providerType] || 1) : config.providerType,
...
function getProviderIcon(type: string | number) {
  const typeName = typeof type === 'number' ? (providerTypeMap[type] || 'Gemini') : type
  switch (typeName) {
      ...
      >
        ...
        <div class="space-y-2 text-sm">
          <div class="flex justify-between">
            ...
            <span class="font-medium">{{ typeof config.providerType === 'number' ? (providerTypeMap[config.providerType] || config.providerType) : config.providerType }}</span>
          </div>
          <div class="flex justify-between">
